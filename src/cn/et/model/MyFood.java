...
public class MyFood {
	...
	public Integer getTableListCount(String name) throws Exception {
		...
		String sql = "select count(rowid) as cr from FOOD where FOODID like '%" + name + "%'";
	...
	public void saveFood(String foodName,String typeid,String price, String imagePath) throws Exception {
		String sql = "insert into food values((select max(typeid)+1 from food),'"+foodName+"','"+typeid+"','"+price+"','"+imagePath+"')";
		DbUtils.execute(sql);
	}
